...
my $package_name  = 'Vector-Object3D';
my $version_from  = 'lib/Vector/Object3D.pm';
...
touch_all_files_in_dir($build_dir);
...
sub get_version_number {
    ...
    my $command = sprintf q{grep -e '$VERSION' %s | awk '{print $4}' | sed -e "s/[';]//g"}, $version_from;
...
sub touch_all_files_in_dir {
    my ($build_dir) = @_;

    printf "\nTouching all files in the build directory:\n\n  * temp dir: %s\n\n", $build_dir;

    my $files = `find \"${build_dir}\" -print`;
    my @files = split /\n/, $files;
